...
var weather = [
	...
	{ type: 'cloud', name: 'Облако' },
	{ type: 'error', name: 'Ошибка' }
];
...
function changeWeather(weather) {
	...
	switch (weather.type) {
		case 'cloud':
			TweenMax.to(settings, 3, { cloudHeight: 200, ease: Power2.easeInOut });
			break;
		default:
			TweenMax.to(settings, 1, { cloudHeight: 100, ease: Power2.easeOut });
			break;
	}

	switch (weather.type) {
		case 'error':
			TweenMax.to(settings, 3, { cloudHeight: 0, cloudHeight: 0,
				cloudSpace: 0,
				cloudArch: 0,
				renewCheck: 0,
				splashBounce: 0, ease: Power2.easeInOut });
			break;
		default:
			TweenMax.to(settings, 1, { cloudHeight: 100,
				cloudSpace: 30,
				cloudArch: 50,
				renewCheck: 10,
				splashBounce: 80, ease: Power2.easeOut });
			break;
	}
	...
	switch (weather.type) {
		case 'sun':
			...
			TweenMax.to(sunburst.node, 4, { scale: 1, opacity: 0.8, x: (sizes.card.width / 2)+ (sizes.card.offset.left), y: (sizes.card.height / 2) + (sizes.card.offset.top), ease: Power2.easeInOut });
			break;
		default:
